...
def get_spectral_contrast(
    ...
    """TODO: adjust this function to generate same output.

    Spectral contrast is a (7,) vector, but in the given data set it is
    squashed into one metric. The current way of dealing with it generates
    the right mean (duh), but the wrong variance.
    """
    audio_data = np.atleast_2d(audio_data)

    spectral_contrasts = []
    spectral_contrast_mean = np.zeros((np.shape(audio_data)[0], 1))
    spectral_contrats_var = np.zeros((np.shape(audio_data)[0], 1))

    for index, row in enumerate(audio_data):
        spectral_contrasts.append(
    ...
    spectral_contrasts = np.atleast_3d(np.array(spectral_contrasts))
    spectral_contrast_squashed = np.mean(np.array(spectral_contrasts), axis=0)

    spectral_contrast_mean = np.atleast_2d(np.mean(spectral_contrast_squashed))
    spectral_contrast_var = np.atleast_2d(np.var(spectral_contrast_squashed))
...
def get_tempo(
    ...
    audio_data = np.atleast_2d(audio_data)
    tempo = np.zeros((np.shape(audio_data)[0], 1))
